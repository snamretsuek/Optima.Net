...
- [Optima.Net Result\<T\>](#optimanet-resultt)
  ...
  - [Guarding Against Null Success Values](#guarding-against-null-success-values)
  - [Integration Example](#integration-example)
  - [Chaining Operations (Bind / BindAsync)](#chaining-operations-bind--bindasync)
  - [Transforming Values (Map / MapAsync)](#transforming-values-map--mapasync)
  - [Side Effects (Tap / TapAsync / OnFailure)](#side-effects-tap--tapasync--onfailure)
...
- [Optima.Net Result\<T,TError\>](#optimanet-resultttterror)
  ...
  - [Mapping and binding](#mapping-and-binding)
  - [Ensure](#ensure)
  - [Async Ensure](#async-ensure)
...
```
var ok = Result\<User, IUserError\>.Ok(user);
var fail = Result\<User, IUserError\>.Fail(new UserNotFound(id));
```
...
```
var r = Result\<int, OrderError\>.Ok(10)
    .Map(x =\> x * 2)
    .Bind(x =\> x \> 5
...
```
Result\<User, IUserError\>.Ok(user)
    .Ensure(u =\> u.IsActive, () =\> new UserInactive());
```
...
```
await Result\<User, IUserError\>.Ok(user)
    .EnsureAsync(async u =\> await repo.Exists(u.Id), () =\> new UserNotFound(u.Id));
```
...
public record CardDeclined : PaymentError;
public record InsufficientFunds : PaymentError;
public record GatewayUnavailable : PaymentError;
```
...
- Value object and aggregate validation
- Application pipelines with clear failure paths
- Mapping domain errors to API ProblemDetails later

Typed Result is Optional with an explicit error lane. It makes intent obvious and code honest.
